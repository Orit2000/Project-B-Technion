...
def load_multi_dt2_data(args):
    ...
    # )
    cache_exists = all(
        os.path.exists(f"Transformer_Map_Interp/cache/{name}set_2_m_train.pt")
        for name in set_configs
    )
    ...
    if cache_exists and (args.new_spread == False):
        ...
        trainset = torch.load(f"Transformer_Map_Interp/cache/trainset_2_m_train.pt", weights_only=False)
        validset = torch.load(f"Transformer_Map_Interp/cache/validset_2_m_train.pt", weights_only=False)
        testset  = torch.load(f"Transformer_Map_Interp/cache/testset_2_m_train.pt",  weights_only=False)
    ...
    # torch.save(testset,  f"Transformer_Map_Interp/cache/testset_{cache_key}.pt")
    torch.save(trainset, f"Transformer_Map_Interp/cache/trainset_2_m_train.pt")
    torch.save(validset, f"Transformer_Map_Interp/cache/validset_2_m_train.pt")
    torch.save(testset,  f"Transformer_Map_Interp/cache/testset_2_m_train.pt")
    #torch.save(calibset, f"Transformer_Map_Interp/cache/calibset_{cache_key}.pt")
