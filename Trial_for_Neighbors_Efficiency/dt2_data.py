...
def faster_add_transformer_masks(
    ...
    if max_radius_km is not None:
        ...
        all_inds_list, all_dist_list = kdt.query_radius(
                                                            q_xy_all,
                                                            r=max_radius_km,
    ...
    for i in range(len(dataset)):
        ...
        dists = all_dist_list[i]

        # --- FAST self-exclude: drop neighbors with distance ~ 0 ---
        if self_exclude:
            mask = dists > 1e-6   # or 1e-9 if you want
            inds = inds[mask]
            dists = dists[mask]   # (if you later use dists)
        ...
        if obs_y_norm.ndim == 2 and obs_y_norm.size(-1) == 1:
            obs_y_norm = obs_y_norm.squeeze(-1)
        obs_coords_norm_tensor[i, :num] = obs_coords_norm
        obs_y_norm_tensor[i, :num]      = obs_y_norm
        ...
        if i % 50000 == 0:
            print(f"Processed {i}/{len(dataset)} samples, neighbors={len(inds)}")
    # # --- finalize per-target tensors (uniform length N) ---
    # if isinstance(dataset.query_coords, list):
    #     dataset.query_coords = torch.stack(
    #         [torch.as_tensor(x, dtype=torch.float32) for x in dataset.query_coords], dim=0
...
def load_dt2_data(args):
    ...
    torch.save(trainset, f"trainset_2_M_points.pt")
    torch.save(validset, f"validset_2_M_point.pt")  # (fix) save validset correctly
    torch.save(testset,  f"testset_2_M_point.pt")
...
def load_multi_dt2_data(args):
    ...
    print("Saving sets...")
    torch.save(trainset, f"trainset_4_M_points_10_nei_new_saving.pt")
    torch.save(validset, f"validset_4_M_points_10_nei_new_saving.pt")
    torch.save(testset,  f"testset_4_M_points_10_nei_new_saving.pt")
    #torch.save(calibset, f"Transformer_Map_Interp/cache/calibset_{cache_key}.pt")
