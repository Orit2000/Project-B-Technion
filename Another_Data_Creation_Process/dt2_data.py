...
def load_multi_dt2_data(args):
    ...
    # cache_exists = all(
    #     os.path.exists(f"Transformer_Map_Interp/cache/{name}set_resampled_10_nei_from_0.05.pt")
    #     for name in set_configs
    # )
    cache_exists = all(
        os.path.exists(f"Transformer_Map_Interp/cache/{name}set_800k_train.pt")
        for name in set_configs
    )
    ...
    if cache_exists and (args.new_spread == False):
        ...
        # validset = torch.load(f"Transformer_Map_Interp/cache/validset_{cache_key}.pt", weights_only=False)
        # testset  = torch.load(f"Transformer_Map_Interp/cache/testset_{cache_key}.pt",  weights_only=False)
        # trainset = torch.load(f"Transformer_Map_Interp/cache/trainset_resampled_10_nei_from_0.05.pt", weights_only=False)
        # validset = torch.load(f"Transformer_Map_Interp/cache/validset_resampled_10_nei_from_0.05.pt", weights_only=False)
        # testset  = torch.load(f"Transformer_Map_Interp/cache/testset_resampled_10_nei_from_0.05.pt",  weights_only=False)
        trainset = torch.load(f"Transformer_Map_Interp/cache/trainset_800k_train.pt", weights_only=False)
        validset = torch.load(f"Transformer_Map_Interp/cache/validset_800k_train.pt", weights_only=False)
        testset  = torch.load(f"Transformer_Map_Interp/cache/testset_800k_train.pt",  weights_only=False)
        print(f"Loaded sets are: train-{len(trainset.coords)}, valid-{len(validset.coords)}, test-{len(testset.coords)}")
        # calibset = torch.load(f"Transformer_Map_Interp/cache/calibset_{cache_key}.pt", weights_only=False)
        return trainset, validset, testset #, calibset
    ...
    # torch.save(testset,  f"Transformer_Map_Interp/cache/testset_{cache_key}.pt")
    torch.save(trainset, f"Transformer_Map_Interp/cache/trainset_800k_train.pt")
    torch.save(validset, f"Transformer_Map_Interp/cache/validset_800k_train.pt")
    torch.save(testset,  f"Transformer_Map_Interp/cache/testset_800k_train.pt")
    #torch.save(calibset, f"Transformer_Map_Interp/cache/calibset_{cache_key}.pt")
